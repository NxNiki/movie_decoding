...
class Wav2Vec2GroupNormConvLayer(nn.Module):
    def __init__(self, config, layer_id=0, image_height: int=10):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else image_height
        self.out_conv_dim = config.conv_dim[layer_id]
...
class Wav2Vec2FeatureEncoder(nn.Module):
    ...
    def __init__(self, config, image_height: int=10):
        super().__init__()

        if config.feat_extract_norm == "group":
            conv_layers = [Wav2Vec2GroupNormConvLayer(config, layer_id=0, image_height=image_height)] + [
...
class CCALayer(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_rest, attention_mask=None, output_attentions=False):
        attn_residual = hidden_states
        hidden_states, attn_weights, _ = self.attention(
            hidden_states, key_value_states=hidden_states_rest, attention_mask=attention_mask, output_attentions=output_attentions
        )
        hidden_states = self.dropout(hidden_states)
        hidden_states = attn_residual + hidden_states
...
class CCAEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.layers = nn.ModuleList([CCALayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
        ...
        for layer in self.layers:
            ...
            if not skip_the_layer or deepspeed_zero3_is_enabled:
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    layer_outputs = layer(
                        hidden_states, hidden_states_rest, attention_mask=attention_mask, output_attentions=output_attentions
                    )
                hidden_states = layer_outputs[0]
...
class CSA(Wav2Vec2PreTrainedModel):
    def __init__(self, config: Wav2Vec2Config, image_height: int=10):
        super().__init__(config)
        self.config = config
        self.feature_extractor = Wav2Vec2FeatureEncoder(config, image_height=image_height)
...
class CCA(Wav2Vec2PreTrainedModel):
...
class MultiEncoder(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.num_labels = config.num_labels
        image_height = config.num_channels
        ...
        for i in range(config.input_channels):
            layer = CSA(config, image_height=self.image_height[i])
    ...
    def forward(
        ...
        for i, layer in enumerate(self.cca_layers):
            cca_output = layer(csa_outputs[i], h_cca[i])
            cca_outputs.append(cca_output[0])
        ...
        # out = sequence_output.mean(dim=0)
        sequence_output = torch.stack([a * output for a, output in zip(nn.functional.softmax(A, dim=0), sequence_output)], dim=0)
