...
class Trainer:
    ...
    def train(self, epochs, fold):
        ...
        for epoch in tqdm(range(epochs)):
            ...
            if (epoch+1) % self.config['validation_step'] == 0:
                ...
                jack_p = stats_m['Jack']
                print('Jack p: ', jack_p, 'overall: ', f'{overall_significant} / {overall_valid}')
                # train.report(
    ...
    def inference(self, fold):
        ...
        for i in range(self.config['num_labels']):
            try:
                ...
                print(self.evaluator.classes[i], ': does not appear in the testing dataset')
        ...
        activation_score_save_path = os.path.join(self.config['test_save_path'],
                                                  'activation_score_fold{}'.format(fold + 1))
    ...
    def permutation(self, fold):
            ...
            for i, concept in enumerate(self.evaluator.classes):
                ...
                if len(concept_indices) > 0:
                    ...
                    for perm in range(permutations):
                        ...
                        sampled_activations = np.random.choice(activation[:, i][mask], size=len(target_activations),
                                                               replace=False)
        ...
        activation_path = os.path.join(self.config['test_save_path'], 'activation_score_fold{}.npy'.format(fold + 1))
    ...
    def memory(self, epoch=-1, phase=1, alongwith=[]):
        ...
        np.save(os.path.join(save_path, 'epoch{}_free_recall_{}_results.npy'.format(epoch, phase)), predictions_all)
        ...
        sts = Permutate(config=self.config, phase=phase, epoch=epoch, alongwith=alongwith, phase_length=predictions_length)
        """method John"""
        # print('***** METHOD JOHN *****')
