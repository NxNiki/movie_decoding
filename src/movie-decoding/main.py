    ...
    for patient, sd, dd in zip(patient_list, sd_list, data_list):
        ...
        for data_type in ["clusterless"]:
            for run in range(5, 6):
                ...
                wandb.login()
                if use_lfp:
                    run_name = "LFP Concept level {} MultiEncoder".format(
                        args["patient"]
                    )
                else:
                    run_name = "Clusterless Concept level {} MultiEncoder".format(
                        args["patient"]
                    )
                wandb.init(
                    project="24_Concepts", name=run_name, reinit=True, entity="24"
                )
