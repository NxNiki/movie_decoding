    ...
    def __init__(self, config):
        ...
        if self.use_lfp:
            ...
            for c, category in enumerate(categories):
                ...
                lfp_path = os.path.join(config['lfp_path'], self.patient, version, 'spectrogram_{}'.format(category.lower()))
        ...
        if self.use_combined:
            self.data = {'clusterless': self.data[0][indices_of_good_samples], 'lfp': self.data[1][indices_of_good_samples]}
        else:
...
def create_weighted_loaders(
    ...
    if p_val > 0:
        ...
        train_dataset = MyDataset(lfp_train, spike_train, label_train, train_indices, transform=transform, pos_weight=train_pos_weights)
        ...
        sampler_train = WeightedRandomSampler(weights=data_weights[train_indices], num_samples=batch_sample_num, replacement=True)
