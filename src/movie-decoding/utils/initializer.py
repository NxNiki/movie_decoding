...
def initialize_inference_dataloaders(config):
    if config['use_sleep']:
        dataset = InferenceDataset(config['data_path'], config['patient'], config['use_lfp'], config['use_spike'],
                                   config['use_bipolar'],
                                   config['use_sleep'], config['free_recall_phase'], config['hour'])
    else:
...
def initialize_model(config): 
    ...
    if config['use_spike']:
        ...
        if config['model_architecture'] == 'multi-vit':
            ...
            spike_model = CCT(img_size=(image_height, image_width),
                                n_input_channels=2,
