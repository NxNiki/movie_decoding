...
FREE_RECALL_SPIKE_TIME = {
    '555_1': (42*60 + 10, 44*60 + 19),
    '562_1': (43*60 + 58, 49*60 + 55),
    '562_2': (4689.152 + 7200.256 * 4 + 3148.032 + 20, 4689.152 + 7200.256 * 4 + 3148.032 + 5 * 60 + 50),
    '564_1': (1*3600 + 30*60 + 59, 1*3600 + 35*60 + 35),
    '564_2': (6428.672 + 7200.256 * 4 + 5063.424 + 2 * 60 + 45, 6428.672 + 7200.256 * 4 + 5063.424 + 7 * 60 + 48)
}
OFFSET = {
    '555_1': 0,
    '562_1': 0,
...
SPIKE_CHANNELS = {
    '555_1': list(np.arange(1, 25)) + list(np.arange(41, 46)) + [47, 66, 68, 74] + list(np.arange(80, 89)),
    '562_1': list(np.arange(1, 25)) + list(np.arange(34, 49)) + list(np.arange(65, 72)) + list(np.arange(73, 81)),
    '562_2': list(np.arange(1, 25)) + list(np.arange(34, 49)) + list(np.arange(65, 72)) + list(np.arange(73, 81)),
    '564_1': [2, 3] + list(np.arange(5, 10)) + list(np.arange(11, 17)) + [34, 36, 39, 41, 42, 44, 45, 47] + list(np.arange(49, 65)) + [69, 76, 77],
    '564_2': [2, 3] + list(np.arange(5, 10)) + list(np.arange(11, 17)) + [34, 36, 39, 41, 42, 44, 45, 47] + list(np.arange(49, 65)) + [69, 76, 77],
}
SPIKE_WINDOWS = {
...
class InferenceDataset(Dataset):
    def __init__(self, data_path, patient, lfp=False, spike=False, bipolar=False, sleep=False, phase=1, hour=1):
        self.spike = spike
        self.lfp = lfp
        self.data_length = 0
        ...
        self.num_samples = int(FREE_RECALL_LFP_TIME[patient + '_' + str(phase)][1] - FREE_RECALL_LFP_TIME[patient + '_' + str(phase)][0]) * self.resolution
        ...
        # if LFP
        if lfp:
            self.sample_range = [(self.alignment_offset + FREE_RECALL_LFP_TIME[patient + '_' + str(phase)][0]) * self.lfp_sf,
                                 (self.alignment_offset + FREE_RECALL_LFP_TIME[patient + '_' + str(phase)][1]) * self.lfp_sf]
            if phase in [1, 2]:
        ...
        if spike:
            self.frame_range = [(self.alignment_offset+FREE_RECALL_SPIKE_TIME[patient + '_' + str(phase)][0]) * self.movie_sampling_rate,
                                (self.alignment_offset+FREE_RECALL_SPIKE_TIME[patient + '_' + str(phase)][1]) * self.movie_sampling_rate]
            self.sorted_channels = SPIKE_CHANNELS[patient + '_' + str(phase)]
            self.channel_path = 'E://projects//Datasets//neuron//spike_data//{}//SortingCombinedSpikes'.format(patient)
            self.bin_len = 1  # ms

            self.spike_times = self.load_channels()
            self.spike_frames = [np.round(i) for i in self.movie_sampling_rate * np.array(self.spike_times, dtype=object) / 1000]
            ...
            for i in range(len(self.spike_frames)):
                in_movie = np.logical_and(self.spike_frames[i] >= self.frame_range[0],
            ...
            for sample in range(self.num_samples):
                ...
                window_left = quotient * self.movie_sampling_rate + windows[remainder][0]
                window_right = quotient * self.movie_sampling_rate + windows[remainder][1]
