...
from src.models.poyo.data import chain, pad, track_batch, track_mask
from src.models.poyo.nn import Embedding, InfiniteVocabEmbedding, PerceiverRotary, compute_loss_or_metric
from src.models.poyo.taxonomy import OutputType, Task
from src.models.poyo.utils import create_linspace_latent_tokens, create_start_end_unit_tokens
...
class POYO(nn.Module):
    def __init__(
        ...
        *,
        ...
        region = ["LTC", "WM", "FC", "CENT", "PARS", "HPC", "FUS", "AMY", "INS", "PHC"]
        self.unit_emb.initialize_vocab(region)
        self.session_emb = InfiniteVocabEmbedding(dim, init_scale=emb_init_scale)
        self.session_emb.initialize_vocab(region)
        self.spike_type_emb = Embedding(8, dim, init_scale=emb_init_scale)
    ...
    def forward(
        ...
        *,
    ...
    ) -> Tuple[Dict[str, TensorType["batch", "*nqueries", "*nchannelsout"]], torch.Tensor, Dict[str, torch.Tensor],]:
        # input
        ...
        if self.using_memory_efficient_attn:
            loss = compute_loss_or_metric("mse", OutputType.CONTINUOUS, output_pred, output_values, output_weights)
        else:
...
class POYOTokenizer:
    ...
    def __call__(self, data):
        ...
        spike_token_type_index = np.concatenate([se_token_type_index, np.zeros_like(spike_unit_index)])
