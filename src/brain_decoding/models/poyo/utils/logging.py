...
logging_.basicConfig(
    ...
    format="%(message)s",
    datefmt="[%X]",
    handlers=[RichHandler(rich_tracebacks=True, markup=True, omit_repeated_times=True, show_path=False)],
)
...
class logging:
    ...
    def __init__(self, *, header: str = None, header_color: str = "black"):
        if header is not None:
            self.header = "[bold {}]{}[/bold {}] ".format(header_color, header, header_color)
        else:
            self.header = ""
...
def set_level_all_loggers(level):
    loggers = [logging_.getLogger(name) for name in logging_.root.manager.loggerDict if name != "rich"]
