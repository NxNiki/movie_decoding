...
from src.models.poyo.nn import RotaryCrossAttention, RotaryEmbedding, RotarySelfAttention
...
class PerceiverRotary(nn.Module):
    ...
    def forward(
        ...
        *,  # (   padded   ) or (   chained   )
        ...
        chained_input = input_seqlen is not None or latent_seqlen is not None

        if padded_input and chained_input:
            raise ValueError(f"Cannot specify both input_mask and " f"input_seqlen/latent_seqlen/output_query_seqlen.")

        if chained_input:
            if input_seqlen is None or latent_seqlen is None:
                raise ValueError(f"Must specify all of input_seqlen, latent_seqlen, " f"output_query_seqlen.")
        ...
        for self_attn, self_ff in self.proc_layers:
            latents = latents + self.dropout(self_attn(latents, latent_timestamp_emb, x_seqlen=latent_seqlen))
