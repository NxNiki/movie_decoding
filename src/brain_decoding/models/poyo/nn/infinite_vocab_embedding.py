...
class InfiniteVocabEmbedding(nn.Module):
    ...
    def __init__(self, embedding_dim, init_scale=0.02):
        ...
        self._register_load_state_dict_pre_hook(self._hook_vocab_on_load_state_dict, with_module=False)
    ...
    def initialize_vocab(self, vocab: List[str]):
        ...
        .. code-block:: python
            ...
            >>> torch.Size([4, 64])
        """
        assert self.vocab is None, f"Vocabulary already initialized, and has {len(self.vocab)} words. "
    ...
    def extend_vocab(self, vocab: List[str], exist_ok=False):
        ...
        # update tokenizer
        self.vocab.update(OrderedDict(zip(vocab, range(len(self.vocab), len(self.vocab) + len(vocab)))))
        ...
        self.weight.data[: len(embeddings_for_existing_words)] = embeddings_for_existing_words
        return self
