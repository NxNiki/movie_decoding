...
from src.models.poyo.nn.rotary_embedding import apply_rotary_pos_emb
    ...
    def __init__(
        ...
        *,
        ...
        if use_memory_efficient_attn and xops is None:
            logging.warning("xformers is not installed, falling back to default attention")
        ...
        *,
        ...
        if self.using_memory_efficient_attn:
            ...
            if query_seqlen is None or context_seqlen is None:
                raise ValueError(f"Both `query_seqlen` and `context_seqlen` must be valid " f"sequence lengths.")
    ...
    def forward(
        ...
        *,
        ...
        if self.using_memory_efficient_attn:
            ...
            if x_seqlen is None:
                raise ValueError(f"`x_seqlen` must be a valid sequence length.")
...
def rotary_memory_efficient_attention(
    *,
    ...
    with torch.no_grad():
        ...
        if q_seqlen is None:
            raise ValueError(f"`q_seqlen` must be a valid sequence length.")
        elif isinstance(q_seqlen, torch.Tensor):
            q_seqlen = q_seqlen.tolist()
        elif not isinstance(q_seqlen, list):
            raise ValueError(f"`q_seqlen` must be a list or a torch.Tensor, " f"got {type(q_seqlen)}")
        ...
        if kv_seqlen is not None:
            ...
            if isinstance(kv_seqlen, torch.Tensor):
                ...
                raise ValueError(f"`kv_seqlen` must be a list or a torch.Tensor, " f"got {type(kv_seqlen)}")
