...
class Evaluator:
    ...
    def roc_analysis(self, y_true, y_pred, label=None):
        ...
        img_path = os.path.join(self.config['valid_save_path'],
    ...
    def calculate_confusion(self, y_true, y_pred, label=None):
        img_path = os.path.join(self.config['valid_save_path'],'fold{}_val_cfmatrix_{}.png'.format(self.fold+1, self.classes[label]))
        ...
        plt.clf()
        plt.close()
        image = wandb.Image(img_path, caption='Fold {} {} Validation Confusion Matrix'.format(self.fold + 1, self.classes[label]))
        return image
