...
class ViTEmbeddings(nn.Module):
    ...
    def forward(
        ...
        batch_size, num_channels, width = pixel_values.shape
        height = None
...
class ViTPatchEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.projection = nn.Conv1d(num_channels, hidden_size, kernel_size=1)
        # self.projection = nn.Linear(num_channels, hidden_size)
...
class ViTModel(nn.Module):
    def __init__(
        self, config, add_pooling_layer: bool = True, use_mask_token: bool = False
    ):
        super().__init__()
        self.config = config
...
class ViTForMaskedImageModeling(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
