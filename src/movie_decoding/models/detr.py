...
class DetrEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [DetrEncoderLayer(config) for _ in range(config.num_hidden_layers)]
        )
...
class DetrDecoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [DetrDecoderLayer(config) for _ in range(config.num_hidden_layers)]
        )
...
class DetrModel(nn.Module):
    def __init__(self, config):
        ...
        self.position_embeddings = DetrSinePositionEmbedding(
            config.hidden_size, normalize=True
        )
    ...
    def forward(
        ...
        if input_mask is None:
            input_mask = torch.ones(
                ((self.config.batch_size, 2 * self.config.num_frame)), device=device
            )
        extract_features = self.feature_extractor(inputs)
        extract_features = extract_features.transpose(1, 2)

        mask = nn.functional.interpolate(
            input_mask[None].float(), size=extract_features.shape[1]
        ).to(torch.bool)[0]
