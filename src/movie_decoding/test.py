...
def method_fdr(
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, "FDR.png"), bbox_inches="tight", dpi=200)
    plt.cla()
    plt.clf()
    plt.close()
...
def perform_memory_test(config, phase="recall1", alongwith=[]):
    ...
    model_dir = os.path.join(
        ...
        model_name,
        "train",
        f"model_weights_epoch{epoch}.tar",
    )
    ...
    save_path = os.path.join(
        ...
        "memory",
        "new",
        f"epoch{epoch}_{phase}_{len(alongwith)}",
    )
    ...
    method_curve_shape(
        ...
        phase,
        save_path,
        use_clusterless=use_clusterless,
        use_lfp=use_lfp,
...
def draw_pvalue_curve(config, phase="recall1", alongwith=[]):
    ...
    plt.tight_layout()
    if len(alongwith) > 0:
        file_path = model_dir.replace("train", "memory").replace(
            f"model_weights_epoch{epoch}.tar",
            f'epoch{epoch}_free_recall_pcurve_{phase}_{"_".join(alongwith)}.png',
        )
    else:
        file_path = model_dir.replace("train", "memory").replace(
            f"model_weights_epoch{epoch}.tar",
            f"epoch{epoch}_free_recall_pcurve_{phase}.png",
        )
    plt.savefig(file_path)
    plt.cla()
    plt.clf()
