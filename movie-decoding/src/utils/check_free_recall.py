...
from scipy.stats import mannwhitneyu, ttest_rel, wilcoxon, ttest_1samp, ttest_ind
...
free_recall_windows_555_FR1 = [
    ...
    [], # white house/DC
    [], # CIA/FBI
    [9563,22429,28464,47244,54959,\
     62789,71765,101644], # hostage/exchange/sacrifice/negotiations
    [], # handcuff/chair/tied
    [12589,61419,106804,112053], # Jack Bauer
    [], # Chloe
    [], # Bill
    [31634,45969,75902,99888,\
     122944], # Abu Fayed (main terrorist)
...
free_recall_windows_562_FR1 = [
    [47894], # LA
    [16662,29149,42753,79223,94616,106387,\
     150762,154253,219886], # attacks/bomb/bus/explosion
    [], # white house/DC
    [9763,210852,273087,300741], # CIA/FBI
    [53874,70537,124121,207739,\
     308822], # hostage/exchange/sacrifice (including negotiation...more commonly what they said)
    [259365], # handcuff/chair/tied
    [62337,74338,137758,139410,223847,\
     248686,286911,321887], # Jack Bauer
    [], # Chloe
    [], # Bill
    [133129,140493,233770,263705,286149,\
     346257], # Abu Fayed (main terrorist)
    [191496,333707,337780,343262,350226], # Ahmed Amar (kid)
    [23949] # President
]
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/562_FR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
...
free_recall_windows_562_FR2 = [
    [50117], # LA
    [19270,45668,49602,75683,199756,\
     239419,317987], # attacks/bomb/bus/explosion
    [60295], # white house/DC
    [62511,105390,258610,284353], # CIA/FBI
    [94940,111449,139963,118219,\
     146279,160157,174136,212141], # hostage/exchange/sacrifice (including negotiation...more commonly what they said)
    [], # handcuff/chair/tied
    [100545,114639,123671,138993,151212,\
     153702,155272,156697,183406,208166,210566], # Jack Bauer
    [], # Chloe
    [], # Bill
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/562_FR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/563_FR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_563_FR2 = [
    ...
    [166527,183570,197798,203470,251550,262637,272425,280108,290238,301530], # Jack Bauer
    [], # Chloe
    [], # Bill
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/563_FR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_564_FR1 = [
    ...
    [], # white house/DC
    [43720,47634], # CIA/FBI
    [97212,136012,186583], # hostage/exchange/sacrifice (including negotiation...more commonly what they said)
    [154512,163743], # handcuff/chair/tied
    [83685,128231,133557,153427,170833,178043,181438,189123,236780,238545], # Jack Bauer
...
free_recall_windows_564_FR2 = [
    ...
    [118506,129104,139320,148855,168405,198367,206866,217996,289288,296563], # Jack Bauer
...
free_recall_windows_566_FR1 = [
    ...
    [16510,55356,68015,78712,100120,117136,193986,206820,224710,250371,263760,278970], # Jack Bauer
    [], # Chloe
    [], # Bill
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/566_FR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_566_CR1 = [
    ...
    [36374,151800,181740,228104,303372,363247,417022,494434], # hostage/exchange/sacrifice
    [345172,357764,387810,396268,404770,410571,428764,436569], # handcuff/chair/tied
    [4934,26904,32845,53553,216072,221191,302160,312600,318530,344745,352170,361366,
    	375642,388110,396090,406890,412800,419056,426493,438630,536346], # Jack Bauer
    [28945,51983,60260,509034], # Chloe
    [], # Bill
    [92005,144240,285912,291158,297776,307333,317590,327078,368423,390960,434610,
    	441689,490350,519359,534002], # Abu Fayed (main terrorist)...calls him "Indu"
    [86379,100395,106426,119420,138696,156971,322300,482898,489930], # Ahmed Amar (kid)
    [175356,194610,203766,212220,222252,465096] # President
]
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/566_CR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_566_FR2 = [
    ...
    [386983,427140,450253,499243,517650,680875,751676], # hostage/exchange/sacrifice
    [412550,660575,673250,791073], # handcuff/chair/tied
    [36977,74164,236531,387448,406041,412430,418460,424308,450797,460271,474942,485645,
        499500,564411,644249,666272,674291,681096,687488,702071,715010,780883,791478,
        808130,819887], # Jack Bauer (calls him Kai/Kite)
    [217637,223420,231768,550686,618514,637550], # Chloe
    [], # Bill
    [505471,511930,525840,531443,572697,579158,589007,599850,682904,716014,722036,
        746304,752562,770731,797900,804242], # Abu Fayed (main terrorist ANDU)
    [268810,291966,346811,358945,730820,736850,743834,753931,760160], # Ahmed Amar (kid)
    [14340,58170,64020] # President
]
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/566_FR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_566_CR2 = [
    ...
    [14400,149869,170773,170773,195180,254640,261324,277350,283989], # Jack Bauer
    [1674,13139,237199], # Chloe
    [320788], # Bill
    [45150,171034,123396,171034,198771,208429], # Abu Fayed (main terrorist...thinks he's ASAAD)
    [73796], # Ahmed Amar (kid)
    [120053,157410] # President
]
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/566_CR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_567_FR1 = [
    ...
    [43170,45930,51056,59820,76260,96072,124230,133500,165082,173280,199200,204643,238524], # Jack Bauer
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/567_FR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_567_CR1 = [
    ...
    [2024,154380,204450,227160,234004,241216,249060,256560,264550,302848,317000], # Jack Bauer
    [1080,6394], # Chloe
    [], # Bill
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/567_CR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_567_FR2 = [
    ...
    [80145,86456,94230,102690,143413,150460,160716,169623,177263,187955,228397,237058,273503,278562], # Jack Bauer
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/567_FR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_567_CR2 = [
    ...
    [100290,137523,175710,185042,191460,198760,205860,214780,224971,256675], # Jack Bauer
    [], # Chloe
    [], # Bill
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/567_CR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/568_FR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_568_CR1 = [
    ...
    [17736,22762,233668,241090,271695,277351,282914,289741,329685,356813], # Jack Bauer
    [13414], # Chloe
    [], # Bill
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/568_CR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/572_FR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_572_CR1 = [
    ...
    [21672,197628,203850,243450,248952,259173,265739,276773,281485], # Jack Bauer
    [6677], # Chloe
    [], # Bill
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/572_CR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_572_FR2 = [
    ...
    [141721,156750,161760,167580,172863,180618,185838,209504,218484,230121,236002], # Jack Bauer
    [260227], # Chloe
    [], # Bill
    [], # Abu Fayed (main terrorist)
    [], # Ahmed Amar (kid)
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/572_FR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_572_CR2 = [
    ...
    [22749,34980,203096,209730,217067,248850,260278,265710,270870,276780], # Jack Bauer
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/572_CR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_i728_FR1a = [
    ...
    [62170,84994,103440,142080,150217,179967,194678,217477,223770,231774,
        248430,353005,361650,370760,385950,398146], # Jack Bauer
    [], # Chloe
    [], # Bill
    [138692,163526,170310,177390,182730,241517,247070,326094,394199,420926], # Abu Fayed (main terrorist)
    [286350,304287,312510,320400], # Ahmed Amar (kid)
    [] # President
]
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/i728_FR1a.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_i728_FR1b = [
    ...
    [73590,98070,114090,119469,131550,139241,159981,168676,175678,183849,197370,209640,215610,\
    225240,251970,264600,276540,291390], # Jack Bauer
    [], # Chloe
    [], # Bill
    [53663,82067,218841,296044,306599,464034,480602], # Abu Fayed (main terrorist)
    [350250,355200,376331,385918,391050,409250,426330,434124,446656,460350,467970,473436], # Ahmed Amar (kid)
    [] # President
]
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/i728_FR1b.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
surrogate_windows_i728_FR1b = np.floor(data[1]).astype(int)
surrogate_windows_i728_FR1b = surrogate_windows_i728_FR1b.tolist()

free_recall_windows_i728_FR1 = [fr1a + [fr1b_item + offset_i728 for fr1b_item in fr1b]  
                                for fr1a, fr1b in zip(free_recall_windows_i728_FR1a, free_recall_windows_i728_FR1b)]
surrogate_windows_i728_FR1 = surrogate_windows_i728_FR1a + [fr1b_item + offset_i728 for fr1b_item in surrogate_windows_i728_FR1b]
...
free_recall_windows_i728_CR1 = [
    ...
    [8916,119366,127470,136860,155736,208135,239019,247266,255986,267715,472470], # Jack Bauer
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/i728_CR1.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_i728_FR2 = [
    ...
    [63107,73980,79746,89137,96457,104100,124140,132734,177145,187080,193680,203183,218190,\
     240330,249254,273420,282690,313290,327450,346590,353250,363129,374280,383730,391410,\
     405750,420660,609310], # Jack Bauer
    [259081], # Chloe
    [], # Bill
    [53268,155534,292560,340740,351690,361530,603336,613840], # Abu Fayed (main terrorist)
    [474450,501990,519120,533370,549010,560693,575220,586431,594630,618970], # Ahmed Amar (kid)
    [114000,186030,257460] # President
]
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/i728_FR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
free_recall_windows_i728_CR2 = [
    ...
    [], # hostage/exchange/sacrifice
    [], # handcuff/chair/tied
    [10860,18193,125261,131070,144810,157410,164670,187358,203357,213960,240885,251820,258570,275777], # Jack Bauer
    [1771], # Chloe
    [], # Bill
...
data = pd.read_csv('/mnt/SSD2/yyding/24/src/utils/annotations/i728_CR2.ann', sep='^([^\s]*)\s', engine='python', header=None)
data[1] = pd.to_numeric(data[1], errors='coerce')
data.dropna(subset=[1], inplace=True)
...
def getEmpiricalConceptPs_yyding(activations, free_recall_windows, bins_back=-4, activation_width=4, surrogate_mask=[]):
    # Updated 2024-02-14
    bin_size=0.25
    permutations=100000
    ...
    for concept_i, concept_vocalizations in enumerate(free_recall_windows):
        if len(concept_vocalizations) > 0:
            ...
            if len(valid_ends) > 0:
                ...
                target_indices = [np.arange(end-(-bins_back+activation_width), end-(-bins_back)) for end in valid_ends]
...
def getEmpiricalConceptPs_hoteling(activations, free_recall_windows, bins_back, activation_width, bin_size=0.25,
                                   permutations=5000):
    time_bins = np.arange(0, len(activations) * bin_size, bin_size)  # all the time bins
    max_time_back = 4
    sig_vector_test = []
    for concept_i, concept_vocalizations in enumerate(free_recall_windows):  # for each concept
        if len(concept_vocalizations) > 0:  # if person said the concept at all
            ...
            for concept_vocalization in concept_vocalizations:  # get the ranges for each concept mention
                ...
                # grab only those bins before concept mention
                temp_tas = []
                if closest_end - (np.max(np.abs(bins_back)) + np.max(activation_width)) >= 0:  # if concept too close to beginning skip it
            ...
            for selected_index in mask_bins:
                ...
                for aw in activation_width:
                    for bb in np.abs(bins_back):
                        if selected_index - (bb + aw) >= 0:  # should have already checked for acceptable bins back but keep JIC
...
def get_rand_idx_with_acceptable_window(mask_bins, win_range_bins, rng=None, max_attempts=100, end_inclusive=True):
    ...
    while attempts < max_attempts:
        ...
        # Generate a range of indices from selected_index - bb to selected_index
        if end_inclusive:
            required_indices = set(range(selected_index + win_range_bins[0], selected_index + win_range_bins[1] + 1))
        else:
            required_indices = set(range(selected_index + win_range_bins[0], selected_index + win_range_bins[1]))
...
def find_random_trial_indices(
    n_rand_trials, all_indicies, win_range_bins, bins_apart_threshold=5, rng=None, with_replacement=False
):
    n_inds = 0
    ...
    while n_to_find > 0:
        ...
        if np.any(too_close_idx):
            too_close_inds = [np.where(too_close_idx)[0][0], np.where(too_close_idx)[0][0] + 1]
        ...
        if len(concept_vocalizations) > 0:  # if person said the concept at all
            ...
            for concept_vocalization in concept_vocalizations:  # get the ranges for each concept mention
                ...
                if closest_end - max_bin_back >= 0:  # if concept too close to beginning skip it
                    ...
                    target_activations_left.append(activations[closest_end-(-bin_back_segments[0][0])+1:closest_end-(-bin_back_segments[0][1])+1, concept_i])
                    target_activations_middle.append(activations[closest_end-(-bin_back_segments[1][0])+1:closest_end-(-bin_back_segments[1][1])+1, concept_i])
                    target_activations_right.append(activations[closest_end-(-bin_back_segments[2][0])+1:closest_end-(-bin_back_segments[2][1])+1, concept_i])
                    target_activations_indices.extend(np.arange(closest_end-max_bin_back, closest_end))  # grab all the indices used in the average not just the bb start
            ...
            # for perm in range(permutations):
            #     selected_index = random.choice(mask_bins)
                surrogate_activations.append(activations[selected_index-max_bin_back+1:selected_index+1, concept_i])
                surrogate_activations_left.append(activations[selected_index-(-bin_back_segments[0][0])+1:selected_index-(-bin_back_segments[0][1])+1, concept_i])
                surrogate_activations_middle.append(activations[selected_index-(-bin_back_segments[1][0])+1:selected_index-(-bin_back_segments[1][1])+1, concept_i])
                surrogate_activations_right.append(activations[selected_index-(-bin_back_segments[2][0])+1:selected_index-(-bin_back_segments[2][1])+1, concept_i])
            ...
            for i in range(len(target_activations_middle)):
                ...
                within_tail_l = likelihood_concept_l <= least_likely_threshold_l #or likelihood_concept_l >= left_threshold_likelihood_l
                mask_most_likely_l = (likelihood_surrogate_l >= np.percentile(likelihood_surrogate_l, high)) # | (likelihood_surrogate_l <= np.percentile(likelihood_surrogate_l, 2))
                ...
                within_tail_m = likelihood_concept_m <= least_likely_threshold_m #or likelihood_concept_m >= left_threshold_likelihood_m
                ...
                within_tail_r = likelihood_concept_r <= least_likely_threshold_r #or likelihood_concept_r >= left_threshold_likelihood_r
                mask_most_likely_r = (likelihood_surrogate_r >= np.percentile(likelihood_surrogate_r, high)) # | (likelihood_surrogate_r <= np.percentile(likelihood_surrogate_r, 2))
                ...
                print(f"{LABELS[concept_i]} {i}th incident is very unlikely to happen:", [within_tail_l, within_tail_m, within_tail_r])
                ...
                if LABELS[concept_i] == 'Jack':
                    ...
                    print('concept presentile: ', np.searchsorted(np.sort(likelihood_surrogate_m), likelihood_concept_m) / len(likelihood_surrogate_m) * 100)
                    print('least likely threshold: ', np.percentile(likelihood_surrogate_m, 10))
                    print('most likely threhold: ', np.percentile(likelihood_surrogate_m, 90))
                    plt.suptitle(f'{LABELS[concept_i]} {i}th')
                    plt.tight_layout()
                    plt.show()
                    plt.close()
...
def GMM_test(activations, free_recall_windows, bin_size=0.25, permutations=5000):
    ...
    for concept_i, concept_vocalizations in enumerate(free_recall_windows):
        if len(concept_vocalizations) <= 0:  # if person said the concept at all
            continue
        for incident_i, concept_vocalization in enumerate(concept_vocalizations):  # get the ranges for each concept mention
            # get the average activation score for each aw window prior to bb
            target_activations = []
            ...
            for interval in bin_back_segments:
                target_activations_segments.append(activations[closest_end-(-interval[0]):closest_end-(-interval[-1])+1, concept_i])
            ...
            for interval in bin_back_segments:
                ...
                # for perm in range(permutations):
                #     selected_index = random.choice(mask_bins)
                    tmp_a.append(activations[selected_index-max_bin_back+1:selected_index+1, concept_i])
                    tmp_as.append(activations[selected_index-(-interval[0]):selected_index-(-interval[-1])+1, concept_i])
            ...
            if len(significant_indices) > 0:
                print(f"{LABELS[concept_i]} {incident_i}th incident is very unlikely to happen in segment {[bin_back_segments[ii] for ii in significant_indices]}")
